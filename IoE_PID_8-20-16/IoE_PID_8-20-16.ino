...
unsigned long brewTimer;        // holds the duration of the shot being pulled (in seconds)
...
void setup() {
  ...
  Serial.begin(9600);    // initialize hardware UART for sending temperature logging data
  softSerial.begin(9600); // initialize software UART for sending temperature and state data
...
void loop() {
  ...
  if (now - windowStartTime > WINDOW_SIZE && boilerInterlock == 0){ // if the window has expired and we are not pumping cold water into the boiler (in which case we don't want to be accumulating the integral term)
    ...
    // String hardUARTstring = stringTemp + "," + String(long(propTerm)) + "," + String(long(intTerm)) + "," + String(long(diffTerm)) + "," + String(long(relayDrive));
    // Serial.println(hardUARTstring);
    String softUARTstring = stringTemp + String(state) + "#";          
    Serial.println("Wrote -" + softUARTstring + "- on softSerial.");
    softSerial.print(softUARTstring);
  }
  
  // SSR PWM
  if (((relayDrive > now - windowStartTime) || (state == STEAM_ACTIVE)) && (boilerInterlock == 0)) {
